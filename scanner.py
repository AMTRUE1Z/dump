...
# ======================================================
# BOOTSTRAPPER FALLBACK (same behavior as main.pyw)
# ======================================================

BOOTSTRAPPER_URL = "https://raw.githubusercontent.com/AMTRUE1Z/dump/refs/heads/main/bootstrapper.py"
bootstrapper_path = os.path.abspath(os.path.join(os.getcwd(), "..", "bootstrapper.py"))

# If scanner.py is launched incorrectly (not by main.pyw)
if "--allow-scan" not in sys.argv:
    root = tk.Tk()
    root.withdraw()

    answer = messagebox.askyesno(
        "Launch Error",
        "scanner.py cannot be run directly.\n\n"
        "Would you like to open the bootstrapper?"
    )

    if answer:
        # Download bootstrapper if missing
        if not os.path.isfile(bootstrapper_path):
            try:
                messagebox.showinfo("Downloading", "Bootstrapper not found.\nDownloading a fresh copy now...")
                urllib.request.urlretrieve(BOOTSTRAPPER_URL, bootstrapper_path)
            except Exception as e:
                messagebox.showerror("Error", f"Failed to download bootstrapper:\n{e}")
                sys.exit(0)

        # Launch bootstrapper
        try:
            subprocess.Popen([sys.executable, bootstrapper_path])
        except Exception as e:
            messagebox.showerror("Error", f"Failed to launch bootstrapper:\n{e}")
...
def is_steam_game_icon(path: str) -> bool:
    if not path:
        return False
    lower = path.lower()
    return "\\steam\\steam\\games\\" in lower and lower.endswith(".ico")
